...
export const SidebarAdmin: React.FC = () => {
  ...
  const menuItems = [
    ...
    { label: 'Stock & Ajustes', path: '/admin/stock', icon: Boxes },
    { label: 'Generador QR', path: '/admin/qr', icon: QrCode },
    { label: 'Vendedores', path: '/admin/vendedores', icon: Users },
    { label: 'Clientes CRM', path: '/admin/clientes', icon: UserCheck },
    { label: 'Ruleta Sorteos', path: '/admin/ruleta', icon: Disc },
    { label: 'Reportes Excel', path: '/admin/reportes', icon: FileSpreadsheet },
  ];
    ...
    <aside className="w-64 glass-panel border-r border-slate-800 min-h-screen flex flex-col justify-between p-4 sticky top-0 h-screen overflow-y-auto">
      <div>
        ...
        <div className="flex items-center gap-3 px-2 py-3 mb-6 border-b border-slate-800">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-teal-500 to-emerald-400 flex items-center justify-center font-bold text-slate-950">
            <Sparkles className="w-5 h-5" />
          </div>
          <div>
            <span className="font-bold text-sm text-slate-100 block">Santa Cruz Essence</span>
            <span className="text-[10px] text-teal-400 font-semibold uppercase tracking-wider">Panel Administrador</span>
          </div>
        </div>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-medium text-sm transition-all ${
                    isActive
                      ? 'bg-gradient-to-r from-teal-900/60 to-emerald-900/40 text-teal-300 border border-teal-500/30 shadow-md shadow-teal-900/20'
